...
AForm::AForm(const std::string name, const int gradeToSign, const int gradeToExecute)
	...
	std::cout << GREEN << "AForm Parameterized constructor called" << NC << std::endl;
}

AForm::AForm(const Form& other)
	: name(other.name), isSigned(other.isSigned), gradeToSign(other.gradeToSign), gradeToExecute(other.gradeToExecute) {
	std::cout << CYAN << "AForm Copy constructor called" << NC << std::endl;
}
...
AForm& AForm::operator=(const AForm& other) {
	...
	std::cout << CYAN << "AForm Copy assignment operator called" << NC << std::endl;
	return *this;
}
